...
_.extend(Channel.prototype, {
  ...
  setRequestHandlers: function(handlers) {
    if (handlers.local) this.handlers.local = handlers.local;
    if (handlers.remote) this.handlers.remote = handlers.remote;
  },
  ...
  active: function(where) {
    if (where == "local") {
      this.api.isLocalEnabled(this.telopt);
    } else if (where === "remote") {
      this.api.isRemoteEnabled(this.telopt);
    }
  }
});
